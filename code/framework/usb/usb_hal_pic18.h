...
#define priInt                  //Define to allow multiple interrupts with USB as Low Prioity
    #if defined(USB_INTERRUPT)
        #if defined(priInt)
            #define USBEnableInterrupts() {RCONbits.IPEN = 1;IPR2bits.USBIP = 0;PIE2bits.USBIE = 1;INTCONbits.GIEH = 1;INTCONbits.GIEL = 1;}
        #else
            #define USBEnableInterrupts() {RCONbits.IPEN = 0;IPR2bits.USBIP = 1;PIE2bits.USBIE = 1;INTCONbits.GIEH = 1;INTCONbits.PEIE = 1;}
        #endif
    #else
        #define USBEnableInterrupts()
    #endif
