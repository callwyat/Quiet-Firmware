...
//    
    ...
    BAUDCON = 0x08;     	// BRG16 = 1
    RCSTA1 = 0x80;       	//Enable Module, Receive Disabled
    
    TXSTA2 = 0x04;          //TX Disabled, BRGH = 1
    BAUDCON2 = 0x08;        //BRG16 = 1
    RCSTA2 = 0x90;          //Enable Module, Continuous Receive
...
void USART_mySetLineCodingHandler(void)
{
    ...
    //Update the baudrate of the UART
        ...
        uint32_t dwBaud;
        dwBaud = ((GetSystemClock()/12) / line_coding.dwDTERate) - 1;
        SPBRG = (uint8_t) dwBaud;
        SPBRGH = (uint8_t)((uint16_t) (dwBaud >> 8));

        dwBaud = ((GetSystemClock()/4) / line_coding.dwDTERate) - 1;
        SPBRG2 = (uint8_t) dwBaud;
        SPBRGH2 = (uint8_t)((uint16_t) (dwBaud >> 8));
